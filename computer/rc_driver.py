...
class ClientOverrideHandler(SocketServer.BaseRequestHandler):
    car = RCControl()
...
class SensorDataHandler(SocketServer.BaseRequestHandler):
    ...
    def handle(self):
        ...
        try:
            while self.data:
                #Only get the latest value and trim anything else off
                self.data = self.request.recv(1024)[0:10]
...
class VideoStreamHandler(SocketServer.StreamRequestHandler):
    ...
    direction_cascade = cv2.CascadeClassifier('cascade_xml/danial.xml')
        ...
        try:
            while True:
                ...
                if first != -1 and last != -1:
                    ...
                    v_param3 = self.obj_detection.detect(self.direction_cascade, gray, image)
                    ...
                    if v_param1 > 0 or v_param2 > 0:
                        ...
                        d3 = self.d_to_camera.calculate(v_param3, self.h3, 300, image)
                    ...
                    # stop conditions
                    if sensor_data is not None and sensor_data < 30 and sensor_data > 5:
...
class ThreadServer(object):
    ...
    def server_thread3(host, port):
        server = SocketServer.TCPServer((host, port), ClientOverrideHandler)
        server.serve_forever()
    ...
    video_thread.start()
    #client_thread = threading.Thread(target=server_thread3('192.168.20.175', 8003))
    #client_thread.start()
